...
async function fetchSongs() {
  try {
    ...
    console.error(e);
    songs = [];
    songsEl.innerHTML = '<div class="muted">Failed to fetch songs.</div>';
  }
}
...
function renderSongs(list = songs) {
  songsEl.innerHTML = '';
  if (!list.length) {
    songsEl.innerHTML = '<div class="muted">No songs found.</div>';
    return;
  }
  ...
  list.forEach(s => {
    ...
    const moods = (s.moods || '').split(',').map(m => m.trim()).filter(Boolean);
    const weatherTags = (s.weatherTags || '').split(',').map(w => w.trim()).filter(Boolean);
    ...
    div.innerHTML = `
      ...
      <div class="meta">
        ...
        <div class="tags">
          ...
          ${moods.map(m => `<span class="tag">${escapeHtml(m)}</span>`).join('')}
          ${weatherTags.map(w => `<span class="tag">${escapeHtml(w)}</span>`).join('')}
        </div>
      </div>
      <div class="actions">
  ...
  document.querySelectorAll('.like').forEach(btn => {
    btn.onclick = () => {
      ...
      btn.textContent = likes.has(btn.dataset.id) ? '❤️' : '🤍';
    };
  });
}
